...
// Création d'une nouvelle liste chainée unilataire vide et renvoie un pointeur sur cette liste
...
int DeleteLinkedListElem(LinkedList* list, SingleLinkedListElem* item) 
{
	...
	{				//il n'y a qu'un seul élement il est donc en tête et en queue
		list->head = NULL;
		list->tail = NULL;
		list->size = 0;
		free(item);
